...
#define MSD_WACOM_STYLUS_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), MSD_TYPE_WACOM_STYLUS, MsdWacomStylusPrivate))
...
static struct {
	...
	MsdWacomRotation rotation_wacom;
	const gchar     *rotation_string;
} rotation_table[] = {
	{ MATE_RR_ROTATION_0,   MSD_WACOM_ROTATION_NONE, "none" },
	{ MATE_RR_ROTATION_90,  MSD_WACOM_ROTATION_CCW,  "ccw"  },
	{ MATE_RR_ROTATION_180, MSD_WACOM_ROTATION_HALF, "half" },
	{ MATE_RR_ROTATION_270, MSD_WACOM_ROTATION_CW,   "cw"   }
};

static WacomDeviceDatabase *db = NULL;

struct MsdWacomStylusPrivate
{
...
static void     msd_wacom_stylus_class_init  (MsdWacomStylusClass *klass);
static void     msd_wacom_stylus_init        (MsdWacomStylus      *wacom_stylus);
static void     msd_wacom_stylus_finalize    (GObject              *object);

G_DEFINE_TYPE (MsdWacomStylus, msd_wacom_stylus, G_TYPE_OBJECT)

static void
msd_wacom_stylus_class_init (MsdWacomStylusClass *klass)
{
        GObjectClass   *object_class = G_OBJECT_CLASS (klass);

        object_class->finalize = msd_wacom_stylus_finalize;

        g_type_class_add_private (klass, sizeof (MsdWacomStylusPrivate));
}

static void
msd_wacom_stylus_init (MsdWacomStylus *stylus)
{
        stylus->priv = MSD_WACOM_STYLUS_GET_PRIVATE (stylus);
}

static void
msd_wacom_stylus_finalize (GObject *object)
{
        MsdWacomStylus *stylus;
        MsdWacomStylusPrivate *p;

        g_return_if_fail (object != NULL);
        g_return_if_fail (MSD_IS_WACOM_STYLUS (object));

        stylus = MSD_WACOM_STYLUS (object);
	...
	p = stylus->priv;
        ...
        G_OBJECT_CLASS (msd_wacom_stylus_parent_class)->finalize (object);
}
...
static MsdWacomStylus *
msd_wacom_stylus_new (MsdWacomDevice    *device,
		      ...
		      GSettings         *settings)
{
	...
	stylus = MSD_WACOM_STYLUS (g_object_new (MSD_TYPE_WACOM_STYLUS,
						 NULL));
...
GSettings *
msd_wacom_stylus_get_settings (MsdWacomStylus *stylus)
{
	g_return_val_if_fail (MSD_IS_WACOM_STYLUS (stylus), NULL);

	return stylus->priv->settings;
}

const char *
msd_wacom_stylus_get_name (MsdWacomStylus *stylus)
{
	g_return_val_if_fail (MSD_IS_WACOM_STYLUS (stylus), NULL);
...
const char *
msd_wacom_stylus_get_icon_name (MsdWacomStylus *stylus)
{
	g_return_val_if_fail (MSD_IS_WACOM_STYLUS (stylus), NULL);
...
MsdWacomDevice *
msd_wacom_stylus_get_device (MsdWacomStylus *stylus)
{
	g_return_val_if_fail (MSD_IS_WACOM_STYLUS (stylus), NULL);
...
gboolean
msd_wacom_stylus_get_has_eraser (MsdWacomStylus *stylus)
{
	g_return_val_if_fail (MSD_IS_WACOM_STYLUS (stylus), FALSE);

	return stylus->priv->has_eraser;
}

guint
msd_wacom_stylus_get_num_buttons (MsdWacomStylus *stylus)
{
	g_return_val_if_fail (MSD_IS_WACOM_STYLUS (stylus), -1);

	return stylus->priv->num_buttons;
}

MsdWacomStylusType
msd_wacom_stylus_get_stylus_type (MsdWacomStylus *stylus)
{
	g_return_val_if_fail (MSD_IS_WACOM_STYLUS (stylus), WACOM_STYLUS_TYPE_UNKNOWN);
...
int
msd_wacom_stylus_get_id (MsdWacomStylus *stylus)
{
	g_return_val_if_fail (MSD_IS_WACOM_STYLUS (stylus), -1);
...
/* Tablet buttons */
static MsdWacomTabletButton *
msd_wacom_tablet_button_new (const char               *name,
			     const char               *id,
			     ...
			     int                       has_oled)
{
	MsdWacomTabletButton *ret;

	ret = g_new0 (MsdWacomTabletButton, 1);
...
void
msd_wacom_tablet_button_free (MsdWacomTabletButton *button)
{
...
msd_wacom_tablet_button_copy (MsdWacomTabletButton *button)
{
	MsdWacomTabletButton *ret;
...
#define MSD_WACOM_DEVICE_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), MSD_TYPE_WACOM_DEVICE, MsdWacomDevicePrivate))
...
struct MsdWacomDevicePrivate
{
	GdkDevice *gdk_device;
	int device_id;
	int opcode;

	MsdWacomDeviceType type;
	char *name;
	char *path;
	char *machine_id;
	const char *icon_name;
...
static void     msd_wacom_device_class_init  (MsdWacomDeviceClass *klass);
static void     msd_wacom_device_init        (MsdWacomDevice      *wacom_device);
static void     msd_wacom_device_finalize    (GObject              *object);

G_DEFINE_TYPE (MsdWacomDevice, msd_wacom_device, G_TYPE_OBJECT)
               ...
               MsdWacomDevice *device)
{
	...
	msd_wacom_device_set_current_stylus (device, tool_id);

	return GDK_FILTER_CONTINUE;
}

static gboolean
setup_property_notify (MsdWacomDevice *device)
{
	Display *dpy;
	XIEventMask evmask;
	int tool_id;
...
static MsdWacomDeviceType
get_device_type (XDeviceInfo *dev)
{
	MsdWacomDeviceType ret;
...
static MateRROutput *
find_output_by_heuristic (MateRRScreen *rr_screen, MsdWacomDevice *device)
{
...
static MateRROutput *
find_output_by_display (MateRRScreen *rr_screen, MsdWacomDevice *device)
{
	gsize n;
	GSettings *tablet;
	GVariant *display;
...
static void
set_display_by_output (MsdWacomDevice  *device,
                       MateRROutput   *rr_output)
{
	GSettings   *tablet;
	GVariant    *c_array;
	...
	tablet  = msd_wacom_device_get_settings (device);
...
void
msd_wacom_device_set_display (MsdWacomDevice *device,
                              int             monitor)
{
	GError *error = NULL;
	     ...
	     MsdWacomDevice *device)
{
	...
	if (rr_output == NULL) {
		if (msd_wacom_device_is_screen_tablet (device)) {
...
int
msd_wacom_device_get_display_monitor (MsdWacomDevice *device)
{
	GError *error = NULL;
	MateRRScreen *rr_screen;
	MateRROutput *rr_output;
	...
	gint area[4];
        ...
        g_return_val_if_fail (MSD_IS_WACOM_DEVICE (device), MSD_WACOM_SET_ALL_MONITORS);

	rr_screen = mate_rr_screen_new (gdk_screen_get_default (), &error);
	if (rr_screen == NULL) {
		g_warning ("Failed to create MateRRScreen: %s", error->message);
		g_error_free (error);
		return MSD_WACOM_SET_ALL_MONITORS;
	}
...
gboolean
msd_wacom_device_get_display_matrix (MsdWacomDevice *device, float matrix[NUM_ELEMS_MATRIX])
{
	int monitor;
	...
	monitor = msd_wacom_device_get_display_monitor (device);
	if (monitor < 0)
		return FALSE;
...
MsdWacomRotation
msd_wacom_device_get_display_rotation (MsdWacomDevice *device)
{
	GError *error = NULL;
	MateRRScreen *rr_screen;
	MateRROutput *rr_output;
...
static void
add_stylus_to_device (MsdWacomDevice *device,
		      const char     *settings_path,
		      int             id)
{
	const WacomStylus *wstylus;
	...
	if (wstylus) {
		...
		if (device->priv->type == WACOM_TYPE_STYLUS) {
			...
			stylus = msd_wacom_stylus_new (device, wstylus, settings);
		} else {
...
int
msd_wacom_device_get_num_modes (MsdWacomDevice *device,
				int             group_id)
{
	int num_modes;

	g_return_val_if_fail (MSD_IS_WACOM_DEVICE (device), -1);
...
int
msd_wacom_device_get_current_mode (MsdWacomDevice *device,
				   int             group_id)
{
	int current_idx;
	...
	/* That means that the mode doesn't exist, see msd_wacom_device_add_modes() */
...
int
msd_wacom_device_set_next_mode (MsdWacomDevice       *device,
				MsdWacomTabletButton *button)
{
	...
	if (num_switches == 1) {
		current_idx = msd_wacom_device_get_current_mode (device, group_id);
		/* msd_wacom_device_get_current_mode() returns -1 when the mode doesn't exist */
		g_return_val_if_fail (current_idx > 0, -1);
...
static GList *
msd_wacom_device_add_ring_modes (WacomDevice      *wacom_device,
				 const char       *settings_path,
				 WacomButtonFlags  direction)
{
	...
	if ((direction & WACOM_BUTTON_POSITION_LEFT) && libwacom_has_ring (wacom_device)) {
		...
		if (num_modes == 0) {
			...
			l = g_list_append (l, msd_wacom_tablet_button_new (_("Left Ring"),
									   "left-ring-mode-1",
									   settings_path,
			...
			for (i = 1; i <= num_modes; i++) {
				...
				l = g_list_append (l, msd_wacom_tablet_button_new (name,
				                                                   id,
				                                                   settings_path,
				                                                   WACOM_TABLET_BUTTON_TYPE_RING,
										   WACOM_TABLET_BUTTON_POS_LEFT,
		...
		if (num_modes == 0) {
			...
			l = g_list_append (l, msd_wacom_tablet_button_new (_("Right Ring"),
									   "right-ring-mode-1",
									   settings_path,
									   WACOM_TABLET_BUTTON_TYPE_RING,
...
static GList *
msd_wacom_device_add_strip_modes (WacomDevice      *wacom_device,
				  const char       *settings_path,
				  WacomButtonFlags  direction)
{
	GList *l;
	...
	if ((direction & WACOM_BUTTON_POSITION_LEFT) && num_strips >= 1) {
		...
		if (num_modes == 0) {
			...
			l = g_list_append (l, msd_wacom_tablet_button_new (_("Left Touchstrip"),
									   "left-strip-mode-1",
									   settings_path,
		...
		if (num_modes == 0) {
			...
			l = g_list_append (l, msd_wacom_tablet_button_new (_("Right Touchstrip"),
									   "right-strip-mode-1",
									   settings_path,
									   WACOM_TABLET_BUTTON_TYPE_STRIP,
									   WACOM_TABLET_BUTTON_POS_RIGHT,
...
static char *
msd_wacom_device_modeswitch_name (WacomButtonFlags flags,
				  guint button_num)
{
...
static MsdWacomTabletButtonType
msd_wacom_device_button_pos (WacomButtonFlags flags)
{
	if (flags & WACOM_BUTTON_POSITION_LEFT)
...
static GList *
msd_wacom_device_add_buttons_dir (WacomDevice      *wacom_device,
				  const char       *settings_path,
				  WacomButtonFlags  direction,
				  ...
				  const char       *button_str_id)
{
	...
	for (i = 'A'; i < 'A' + num_buttons; i++) {
		...
		l = g_list_append (l, msd_wacom_tablet_button_new (name,
		                                                   ...
		                                                   msd_wacom_device_button_pos (flags),
	...
	for (i = 'A'; i < 'A' + num_buttons; i++) {
		...
		name = msd_wacom_device_modeswitch_name (flags, button_num++);
		id = g_strdup_printf ("%s%c", button_str_id, i);
	...
	if (libwacom_has_ring2 (wacom_device) || libwacom_has_ring (wacom_device))
		l = g_list_concat (l, msd_wacom_device_add_ring_modes (wacom_device, settings_path, direction));
	if  (libwacom_get_num_strips (wacom_device) > 0)
		l = g_list_concat (l, msd_wacom_device_add_strip_modes (wacom_device, settings_path, direction));

	return l;
}

static void
msd_wacom_device_add_buttons (MsdWacomDevice *device,
			      ...
			      const char     *settings_path)
{
	...
	l = msd_wacom_device_add_buttons_dir (wacom_device, settings_path, WACOM_BUTTON_POSITION_LEFT, _("Left Button #%d"), "button");
	if (l)
		ret = l;
	l = msd_wacom_device_add_buttons_dir (wacom_device, settings_path, WACOM_BUTTON_POSITION_RIGHT, _("Right Button #%d"), "button");
	if (l)
		ret = g_list_concat (ret, l);
	l = msd_wacom_device_add_buttons_dir (wacom_device, settings_path, WACOM_BUTTON_POSITION_TOP, _("Top Button #%d"), "button");
	if (l)
		ret = g_list_concat (ret, l);
	l = msd_wacom_device_add_buttons_dir (wacom_device, settings_path, WACOM_BUTTON_POSITION_BOTTOM, _("Bottom Button #%d"), "button");
	if (l)
		ret = g_list_concat (ret, l);
...
static void
msd_wacom_device_get_modeswitches (WacomDevice      *wacom_device,
				   gint             *num_rings,
				   gint             *num_strips)
{
	*num_strips = libwacom_get_num_strips (wacom_device);
...
static void
msd_wacom_device_add_modes (MsdWacomDevice *device,
			    WacomDevice    *wacom_device)
{
	GList *l;
	...
	for (l = device->priv->buttons; l != NULL; l = l->next) {
		MsdWacomTabletButton *button = l->data;
...
static void
msd_wacom_device_update_from_db (MsdWacomDevice *device,
				 WacomDevice    *wacom_device,
				 const char     *identifier)
{
	...
	if (device->priv->type == WACOM_TYPE_PAD) {
		msd_wacom_device_get_modeswitches (wacom_device,
						   &device->priv->num_rings,
						   &device->priv->num_strips);
		msd_wacom_device_add_buttons (device, wacom_device, settings_path);
		msd_wacom_device_add_modes (device, wacom_device);
	}
...
static GObject *
msd_wacom_device_constructor (GType                     type,
                              ...
                              GObjectConstructParam     *construct_properties)
{
        ...
        device = MSD_WACOM_DEVICE (G_OBJECT_CLASS (msd_wacom_device_parent_class)->constructor (type,
	...
	msd_wacom_device_update_from_db (device, wacom_device, device->priv->path);
...
static void
msd_wacom_device_set_property (GObject        *object,
                               guint           prop_id,
                               const GValue   *value,
                               GParamSpec     *pspec)
{
        ...
        device = MSD_WACOM_DEVICE (object);
...
static void
msd_wacom_device_get_property (GObject        *object,
                               guint           prop_id,
                               GValue         *value,
                               GParamSpec     *pspec)
{
...
static void
msd_wacom_device_class_init (MsdWacomDeviceClass *klass)
{
        GObjectClass   *object_class = G_OBJECT_CLASS (klass);

        object_class->constructor = msd_wacom_device_constructor;
        object_class->finalize = msd_wacom_device_finalize;
        object_class->set_property = msd_wacom_device_set_property;
        object_class->get_property = msd_wacom_device_get_property;

        g_type_class_add_private (klass, sizeof (MsdWacomDevicePrivate));
...
static void
msd_wacom_device_init (MsdWacomDevice *device)
{
        char *per_user_config;

        device->priv = MSD_WACOM_DEVICE_GET_PRIVATE (device);
        device->priv->type = WACOM_TYPE_INVALID;
...
static void
msd_wacom_device_finalize (GObject *object)
{
        MsdWacomDevice *device;
        MsdWacomDevicePrivate *p;

        g_return_if_fail (object != NULL);
        g_return_if_fail (MSD_IS_WACOM_DEVICE (object));
	...
	p = device->priv;
        ...
        g_list_foreach (p->buttons, (GFunc) msd_wacom_tablet_button_free, NULL);
	...
	gdk_window_remove_filter (NULL,
        ...
        G_OBJECT_CLASS (msd_wacom_device_parent_class)->finalize (object);
}

MsdWacomDevice *
msd_wacom_device_new (GdkDevice *device)
{
	return MSD_WACOM_DEVICE (g_object_new (MSD_TYPE_WACOM_DEVICE,
					       "gdk-device", device,
					       NULL));
}

GList *
msd_wacom_device_list_styli (MsdWacomDevice *device)
{
	g_return_val_if_fail (MSD_IS_WACOM_DEVICE (device), NULL);

	return g_list_copy (device->priv->styli);
}

MsdWacomStylus *
msd_wacom_device_get_stylus_for_type (MsdWacomDevice     *device,
				      MsdWacomStylusType  type)
{
	...
	for (l = device->priv->styli; l != NULL; l = l->next) {
		MsdWacomStylus *stylus = l->data;

		if (msd_wacom_stylus_get_stylus_type (stylus) == type)
			return stylus;
	}
	return NULL;
}
...
const char *
msd_wacom_device_get_name (MsdWacomDevice *device)
{
	g_return_val_if_fail (MSD_IS_WACOM_DEVICE (device), NULL);
...
const char *
msd_wacom_device_get_layout_path (MsdWacomDevice *device)
{
	g_return_val_if_fail (MSD_IS_WACOM_DEVICE (device), NULL);
...
const char *
msd_wacom_device_get_path (MsdWacomDevice *device)
{
	g_return_val_if_fail (MSD_IS_WACOM_DEVICE (device), NULL);
...
const char *
msd_wacom_device_get_icon_name (MsdWacomDevice *device)
{
	g_return_val_if_fail (MSD_IS_WACOM_DEVICE (device), NULL);
...
const char *
msd_wacom_device_get_tool_name (MsdWacomDevice *device)
{
	g_return_val_if_fail (MSD_IS_WACOM_DEVICE (device), NULL);
...
gboolean
msd_wacom_device_reversible (MsdWacomDevice *device)
{
	g_return_val_if_fail (MSD_IS_WACOM_DEVICE (device), FALSE);

	return device->priv->reversible;
}

gboolean
msd_wacom_device_is_screen_tablet (MsdWacomDevice *device)
{
	g_return_val_if_fail (MSD_IS_WACOM_DEVICE (device), FALSE);
...
gboolean
msd_wacom_device_is_isd (MsdWacomDevice *device)
{
	g_return_val_if_fail (MSD_IS_WACOM_DEVICE (device), FALSE);
...
gboolean
msd_wacom_device_is_fallback (MsdWacomDevice *device)
{
	g_return_val_if_fail (MSD_IS_WACOM_DEVICE (device), FALSE);
...
gint
msd_wacom_device_get_num_strips (MsdWacomDevice *device)
{
	g_return_val_if_fail (MSD_IS_WACOM_DEVICE (device), 0);

	return device->priv->num_strips;
}

gint
msd_wacom_device_get_num_rings (MsdWacomDevice *device)
{
	g_return_val_if_fail (MSD_IS_WACOM_DEVICE (device), 0);
...
GSettings *
msd_wacom_device_get_settings (MsdWacomDevice *device)
{
	g_return_val_if_fail (MSD_IS_WACOM_DEVICE (device), NULL);
...
void
msd_wacom_device_set_current_stylus (MsdWacomDevice *device,
				     int             stylus_id)
{
	GList *l;
	MsdWacomStylus *stylus;
	...
	if (device->priv->last_stylus != NULL) {
		MsdWacomStylus *stylus = device->priv->last_stylus;
...
MsdWacomDeviceType
msd_wacom_device_get_device_type (MsdWacomDevice *device)
{
	g_return_val_if_fail (MSD_IS_WACOM_DEVICE (device), WACOM_TYPE_INVALID);

	return device->priv->type;
}

gint *
msd_wacom_device_get_area (MsdWacomDevice *device)
{
	int i, id;
	XDevice *xdevice;
	Atom area, realtype;
...
gint *
msd_wacom_device_get_default_area (MsdWacomDevice *device)
{
	int id;
	gint *device_area;
	gboolean ret;
...
const char *
msd_wacom_device_type_to_string (MsdWacomDeviceType type)
{
	switch (type) {
...
GList *
msd_wacom_device_get_buttons (MsdWacomDevice *device)
{
	g_return_val_if_fail (MSD_IS_WACOM_DEVICE (device), NULL);
...
static MsdWacomTabletButton *
find_button_with_id (MsdWacomDevice *device,
		     const char     *id)
{
	GList *l;
...
static MsdWacomTabletButton *
find_button_with_index (MsdWacomDevice *device,
			const char     *id,
			int             index)
{
	MsdWacomTabletButton *button;
...
MsdWacomTabletButton *
msd_wacom_device_get_button (MsdWacomDevice   *device,
			     int               button,
...
MsdWacomRotation
msd_wacom_device_rotation_name_to_type (const char *rotation)
{
        guint i;

	g_return_val_if_fail (rotation != NULL, MSD_WACOM_ROTATION_NONE);
...
const char *
msd_wacom_device_rotation_type_to_name (MsdWacomRotation type)
{
        guint i;
...
MsdWacomDevice *
msd_wacom_device_create_fake (MsdWacomDeviceType  type,
			      const char         *name,
			      const char         *tool_name)
{
	...
	device = MSD_WACOM_DEVICE (g_object_new (MSD_TYPE_WACOM_DEVICE, NULL));
	...
	msd_wacom_device_update_from_db (device, wacom_device, name);
	libwacom_destroy (wacom_device);
...
GList *
msd_wacom_device_create_fake_cintiq (void)
{
	MsdWacomDevice *device;
	GList *devices;

	device = msd_wacom_device_create_fake (WACOM_TYPE_STYLUS,
					       "Wacom Cintiq 21UX2",
					       "Wacom Cintiq 21UX2 stylus");
	devices = g_list_prepend (NULL, device);

	device = msd_wacom_device_create_fake (WACOM_TYPE_ERASER,
					       "Wacom Cintiq 21UX2",
					       "Wacom Cintiq 21UX2 eraser");
	devices = g_list_prepend (devices, device);

	device = msd_wacom_device_create_fake (WACOM_TYPE_PAD,
					       "Wacom Cintiq 21UX2",
...
GList *
msd_wacom_device_create_fake_bt (void)
{
	MsdWacomDevice *device;
	GList *devices;
	...
	device = msd_wacom_device_create_fake (WACOM_TYPE_CURSOR,
					       "Wacom Graphire Wireless",
...
GList *
msd_wacom_device_create_fake_x201 (void)
{
	MsdWacomDevice *device;
	GList *devices;
...
GList *
msd_wacom_device_create_fake_intuos4 (void)
{
	MsdWacomDevice *device;
	GList *devices;
