 ...
 *
 * The code is derived from msd-wacom-led-helper.c
 * written by:
...
static void
msd_wacom_oled_convert_1_bit (guchar *image)
{
...
static int
msd_wacom_oled_prepare_buf (guchar *image, MsdWacomOledType type)
{
	int len = 0;

	switch (type) {
	case MSD_WACOM_OLED_TYPE_USB:
		/* Image has to be scrambled for devices connected over USB ... */
		oled_scramble_icon (image);
		len = USB_BUF_LEN;
		break;
	case MSD_WACOM_OLED_TYPE_BLUETOOTH:
		/* ... but for bluetooth it has to be converted to 1 bit colour instead of scrambling */
		msd_wacom_oled_convert_1_bit (image);
		len = BT_BUF_LEN;
		break;
	case MSD_WACOM_OLED_TYPE_RAW_BLUETOOTH:
...
static gboolean
msd_wacom_oled_helper_write (const gchar *filename, gchar *buffer, MsdWacomOledType type, GError **error)
{
	guchar *image;
	gint retval;
	gsize length;
	...
	length = msd_wacom_oled_prepare_buf (image, type);
	if (!length) {
		ret = FALSE;
		   ...
		   MsdWacomOledType *type)
{
	...
	if (parent) {
		...
		if(g_file_test (filename, G_FILE_TEST_EXISTS)) {
			*type = usb ? MSD_WACOM_OLED_TYPE_USB : MSD_WACOM_OLED_TYPE_RAW_BLUETOOTH;
			return filename;
		}
	...
	if (usb) {
		...
		*type = MSD_WACOM_OLED_TYPE_BLUETOOTH;
	} else {
...
int main (int argc, char **argv)
{
	...
	if (msd_wacom_oled_helper_write (filename, buffer, type, &error) == FALSE) {
