		  ...
		  MsdWacomRotation  rotation)
{
	...
	cr = cairo_create (surface);
         ...
         */
        if (rotation == MSD_WACOM_ROTATION_HALF ||
            rotation == MSD_WACOM_ROTATION_CCW) {
...
char *
msd_wacom_oled_gdkpixbuf_to_base64 (GdkPixbuf *pixbuf)
{
	int i, x, y, ch, rs;
	guchar *pix, *p;
	guchar *image;
...
void
set_oled (MsdWacomDevice	*device,
	  char			*button_id,
	  char			*label)
{
	...
	command = g_strdup_printf ("pkexec " LIBEXECDIR "/msd-wacom-oled-helper --path %s --button %d --buffer %s",
