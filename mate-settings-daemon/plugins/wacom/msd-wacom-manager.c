...
#include "msd-wacom-oled.h"
#include "msd-wacom-osd-window.h"
#include "msd-shell-helper.h"
#include "msd-device-mapper.h"

#define MSD_WACOM_MANAGER_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), MSD_TYPE_WACOM_MANAGER, MsdWacomManagerPrivate))
...
#define MSD_WACOM_DBUS_PATH MSD_DBUS_PATH "/Wacom"
#define MSD_WACOM_DBUS_NAME MSD_DBUS_NAME ".Wacom"
...
struct MsdWacomManagerPrivate
{
        guint start_idle_id;
        GdkDeviceManager *device_manager;
        guint device_added_id;
        guint device_removed_id;
        GHashTable *devices; /* key = GdkDevice, value = MsdWacomDevice */
        MateRRScreen *rr_screen;
        GHashTable *warned_devices;

        MsdShell *shell_proxy;

        MsdDeviceMapper *device_mapper;
...
static void     msd_wacom_manager_class_init  (MsdWacomManagerClass *klass);
static void     msd_wacom_manager_init        (MsdWacomManager      *wacom_manager);
static void     msd_wacom_manager_finalize    (GObject              *object);

static gboolean osd_window_toggle_visibility (MsdWacomManager *manager,
                                              MsdWacomDevice  *device);

static MsdWacomDevice * device_id_to_device (MsdWacomManager *manager,
                                             int              deviceid);

static void             osd_window_destroy  (MsdWacomManager *manager);

G_DEFINE_TYPE (MsdWacomManager, msd_wacom_manager, G_TYPE_OBJECT)

static gpointer manager_object = NULL;

static void
msd_wacom_manager_class_init (MsdWacomManagerClass *klass)
{
        GObjectClass   *object_class = G_OBJECT_CLASS (klass);

        object_class->finalize = msd_wacom_manager_finalize;

        g_type_class_add_private (klass, sizeof (MsdWacomManagerPrivate));
}

static int
get_device_id (MsdWacomDevice *device)
{
...
static XDevice *
open_device (MsdWacomDevice *device)
{
	XDevice *xdev;
	int id;
...
static gboolean
set_osd_visibility (MsdWacomManager *manager,
		    guint            device_id,
		    gboolean         visible,
		    gboolean         edition_mode)
{
	...
	if (manager->priv->osd_window &&
	    device != msd_wacom_osd_window_get_device (MSD_WACOM_OSD_WINDOW (manager->priv->osd_window)))
		osd_window_destroy (manager);
	...
	msd_wacom_osd_window_set_edition_mode (MSD_WACOM_OSD_WINDOW (manager->priv->osd_window),
					       edition_mode);
...
handle_method_call (GDBusConnection       *connection,
                    ...
                    gpointer               data)
{
	MsdWacomManager *self = MSD_WACOM_MANAGER (data);
...
static void
wacom_set_property (MsdWacomDevice *device,
		    PropertyHelper *property)
{
	XDevice *xdev;

	xdev = open_device (device);
	device_set_property (xdev, msd_wacom_device_get_tool_name (device), property);
	xdevice_close (xdev);
}

static void
set_pressurecurve (MsdWacomDevice *device,
                   GVariant       *value)
{
        PropertyHelper property = {
 ...
 */
static void
set_area (MsdWacomDevice  *device,
          GVariant        *value)
{
        PropertyHelper property = {
        ...
        if (property.data.i[0] == -1 &&
            ...
            property.data.i[3] == -1) {
                gint *area;
                area = msd_wacom_device_get_default_area (device);
...
static void
reset_area (MsdWacomDevice *device)
{
...
static void
set_absolute (MsdWacomDevice  *device,
              gint             is_absolute)
{
	XDevice *xdev;
	...
	if (gdk_error_trap_pop ())
		g_error ("Failed to set mode \"%s\" for \"%s\".",
			 is_absolute ? "Absolute" : "Relative", msd_wacom_device_get_tool_name (device));
	xdevice_close (xdev);
}
                     ...
                     MsdWacomRotation rotation)
{
	...
	if (rotation == MSD_WACOM_ROTATION_CW || rotation == MSD_WACOM_ROTATION_CCW)
...
static void
set_keep_aspect (MsdWacomDevice *device,
                 gboolean        keep_aspect)
{
        GVariant *values[4], *variant;
	guint i;
	GdkDevice *gdk_device;
	MsdDeviceMapper *mapper;
	gint *area;
	gint monitor = MSD_WACOM_SET_ALL_MONITORS;
	MsdWacomRotation rotation;
	GSettings *settings;

        settings = msd_wacom_device_get_settings (device);
	...
	/* Get current area */
	area = msd_wacom_device_get_area (device);
	if (!area) {
	...
	mapper = msd_device_mapper_get ();
	g_object_get (device, "gdk-device", &gdk_device, NULL);
	monitor = msd_device_mapper_get_device_monitor (mapper, gdk_device);
...
static void
set_device_buttonmap (MsdWacomDevice *device,
                      GVariant       *value)
{
	XDevice *xdev;
	gsize nmap;
	...
	if ((gdk_error_trap_pop () && rc != MappingSuccess) ||
	    rc != MappingSuccess)
		g_warning ("Error in setting button mapping for \"%s\"", msd_wacom_device_get_tool_name (device));
...
static void
set_touch (MsdWacomDevice *device,
	   gboolean        touch)
{
        gchar data = touch;
        PropertyHelper property = {
...
static void
set_pressurethreshold (MsdWacomDevice *device,
                       gint            threshold)
{
        PropertyHelper property = {
...
static void
apply_stylus_settings (MsdWacomDevice *device)
{
	...
	g_debug ("Applying setting for stylus '%s' on device '%s'",
		 msd_wacom_stylus_get_name (stylus),
		 msd_wacom_device_get_name (device));

	stylus_settings = msd_wacom_stylus_get_settings (stylus);
	 ...
	 int                   index)
{
	...
	path = msd_wacom_device_get_path (device);
	status_led = button->status_led;

	if (status_led == MSD_WACOM_NO_LED) {
		g_debug ("Ignoring unhandled group ID %d for device %s",
		         button->group_id, msd_wacom_device_get_name (device));
		return;
	}
	g_debug ("Switching group ID %d to index %d for device %s", button->group_id, index, path);

	command = g_strdup_printf ("pkexec " LIBEXECDIR "/msd-wacom-led-helper --path %s --group %d --led %d",
...
static void
reset_pad_buttons (MsdWacomDevice *device)
{
	XDevice *xdev;
	int nmap;
	unsigned char *map;
	...
	if ((gdk_error_trap_pop () && rc != MappingSuccess) ||
	    rc != MappingSuccess)
		g_warning ("Error in resetting button mapping for \"%s\" (rc=%d)", msd_wacom_device_get_tool_name (device), rc);
...
static void
set_wacom_settings (MsdWacomManager *manager,
		    MsdWacomDevice  *device)
{
	...
	g_debug ("Applying settings for device '%s' (type: %s)",
		 msd_wacom_device_get_tool_name (device),
		 msd_wacom_device_type_to_string (msd_wacom_device_get_device_type (device)));

	settings = msd_wacom_device_get_settings (device);
        set_touch (device, g_settings_get_boolean (settings, KEY_TOUCH));

        type = msd_wacom_device_get_device_type (device);

	if (type == WACOM_TYPE_TOUCH &&
	    msd_wacom_device_is_screen_tablet (device) == FALSE) {
	...
	if (type != WACOM_TYPE_TOUCH) {
		if (msd_wacom_device_is_screen_tablet (device) == FALSE)
			...
			MsdWacomDevice *device)
{
	...
	if (g_str_equal (key, KEY_ROTATION)) {
                /* Real device rotation is handled in MsdDeviceMapper */
		if (type == WACOM_TYPE_PAD)
			update_pad_leds (device);
		...
		if (type != WACOM_TYPE_CURSOR &&
			...
			set_area (device, g_settings_get_value (settings, key));
                ...
                /* Unhandled, MsdDeviceMapper handles this */
	} else if (g_str_equal (key, KEY_KEEP_ASPECT)) {
		if (type != WACOM_TYPE_CURSOR &&
		    type != WACOM_TYPE_PAD &&
		    type != WACOM_TYPE_TOUCH &&
		    !msd_wacom_device_is_screen_tablet (device))
			 ...
			 MsdWacomStylus *stylus)
{
	...
	device = msd_wacom_stylus_get_device (stylus);
	...
	if (last_stylus != stylus) {
		g_debug ("Not applying changed settings because '%s' is the current stylus, not '%s'",
			 last_stylus ? msd_wacom_stylus_get_name (last_stylus) : "NONE",
			 msd_wacom_stylus_get_name (stylus));
		return;
	}
...
static void
last_stylus_changed (MsdWacomDevice  *device,
		     GParamSpec      *pspec,
...
static void
osd_window_destroy (MsdWacomManager *manager)
{
                               ...
                               MsdWacomManager *manager)
{
	...
	if (msd_wacom_osd_window_get_edition_mode (osd_window))
		return FALSE;
...
static gboolean
osd_window_toggle_visibility (MsdWacomManager *manager,
                              MsdWacomDevice  *device)
{
	GtkWidget *widget;
	...
	if (manager->priv->osd_window) {
        ...
        layout_path = msd_wacom_device_get_layout_path (device);
        if (layout_path == NULL) {
	...
	widget = msd_wacom_osd_window_new (device, NULL);
...
static gboolean
osd_window_update_viewable (MsdWacomManager      *manager,
                            MsdWacomTabletButton *button,
                            ...
                            XIEvent              *xiev)
{
	...
	msd_wacom_osd_window_set_active (MSD_WACOM_OSD_WINDOW (manager->priv->osd_window),
	                                 button,
	                                 dir,
...
static void
notify_unknown_device (MsdWacomManager *manager, const gchar *device_name)
{
        gchar *msg_body;
                 ...
                 MsdWacomManager  *manager)
{
	...
	device = msd_wacom_device_new (gdk_device);
	device_name = msd_wacom_device_get_name (device);
	type = msd_wacom_device_get_device_type (device);

        if (msd_wacom_device_is_fallback (device) &&
	...
	g_debug ("Adding device '%s' (type: '%s') to known devices list",
		 msd_wacom_device_get_tool_name (device),
		 msd_wacom_device_type_to_string (type));
	...
	if (type == WACOM_TYPE_PAD ||
	    ...
	    type == WACOM_TYPE_ERASER ||
	    (type == WACOM_TYPE_TOUCH && !msd_wacom_device_is_screen_tablet (device))) {
		msd_device_mapper_add_input (manager->priv->device_mapper,
					     gdk_device, settings);
	}
	...
	if (type == WACOM_TYPE_STYLUS || type == WACOM_TYPE_ERASER) {
		...
		styli = msd_wacom_device_list_styli (device);

		for (l = styli ; l ; l = l->next) {
			settings = msd_wacom_stylus_get_settings (l->data);
			g_signal_connect (G_OBJECT (settings), "changed",
                   ...
                   MsdWacomManager  *manager)
{
	...
	g_hash_table_remove (manager->priv->devices, gdk_device);
        ...
        msd_device_mapper_remove_input (manager->priv->device_mapper,
                                        gdk_device);
...
device_id_to_device (MsdWacomManager *manager,
		     int              deviceid)
{
	GList *devices, *l;
	MsdWacomDevice *ret;
...
static void
generate_key (MsdWacomTabletButton *wbutton,
	      int                   group,
	      Display              *display,
	      GtkDirectionType      dir,
	      gboolean              is_press)
...
static void
switch_monitor (MsdWacomManager *manager,
                MsdWacomDevice *device)
{
	...
	if (n_monitors < 2)
        ...
        g_object_get (device, "gdk-device", &gdk_device, NULL);
        current_monitor =
                msd_device_mapper_get_device_monitor (manager->priv->device_mapper,
                                                      gdk_device);
	...
	if (current_monitor >= n_monitors)
        ...
        msd_device_mapper_set_device_monitor (manager->priv->device_mapper,
                                              gdk_device, current_monitor);
}

static void
notify_osd_for_device (MsdWacomManager *manager,
                       MsdWacomDevice  *device)
{
        GdkDevice *gdk_device;
        GdkScreen *screen;
        ...
        g_object_get (device, "gdk-device", &gdk_device, NULL);
        monitor_num = msd_device_mapper_get_device_monitor (manager->priv->device_mapper,
                                                            gdk_device);

        if (monitor_num == MSD_WACOM_SET_ALL_MONITORS)
                return;
        ...
        shell_show_osd (manager->priv->shell_proxy,
                        ...
                        msd_wacom_device_get_name (device), -1,
                        monitor_num);
}

static const char*
get_direction_name (MsdWacomTabletButtonType type,
                    GtkDirectionType         dir)
{
                      ...
                      MsdWacomManager *manager)
{
	...
	wbutton = msd_wacom_device_get_button (device, button, &dir);
	if (wbutton == NULL) {
		g_warning ("Could not find matching button for '%d' on '%s'",
			   button, msd_wacom_device_get_name (device));
		return GDK_FILTER_CONTINUE;
	}
	...
	if (wbutton->type == WACOM_TABLET_BUTTON_TYPE_HARDCODED) {
		...
		new_mode = msd_wacom_device_set_next_mode (device, wbutton);
                if (manager->priv->osd_window != NULL) {
			msd_wacom_osd_window_set_mode (MSD_WACOM_OSD_WINDOW(manager->priv->osd_window), wbutton->group_id, new_mode);
			osd_window_update_viewable (manager, wbutton, dir, xiev);
                }
	...
	if (manager->priv->osd_window != NULL) {
		MsdWacomDevice *osd_window_device;
		gboolean edition_mode;
	...
	/* Nothing to do */
	if (g_settings_get_enum (wbutton->settings, KEY_ACTION_TYPE) == MSD_WACOM_ACTION_TYPE_NONE)
		return GDK_FILTER_REMOVE;

	/* Show OSD window when requested */
	if (g_settings_get_enum (wbutton->settings, KEY_ACTION_TYPE) == MSD_WACOM_ACTION_TYPE_HELP) {
		if (xiev->evtype == XI_ButtonRelease)
	...
	/* Switch monitor */
	if (g_settings_get_enum (wbutton->settings, KEY_ACTION_TYPE) == MSD_WACOM_ACTION_TYPE_SWITCH_MONITOR) {
		if (xiev->evtype == XI_ButtonRelease) {
...
static void
set_devicepresence_handler (MsdWacomManager *manager)
{
        GdkDeviceManager *device_manager;
...
static void
msd_wacom_manager_init (MsdWacomManager *manager)
{
        manager->priv = MSD_WACOM_MANAGER_GET_PRIVATE (manager);
}

static gboolean
msd_wacom_manager_idle_cb (MsdWacomManager *manager)
{
	GList *devices, *l;

        mate_settings_profile_start (NULL);

        manager->priv->device_mapper = msd_device_mapper_get ();
...
on_rr_screen_acquired (GObject      *object,
                       ...
                       gpointer      user_data)
{
        MsdWacomManager *manager = user_data;
        GError *error = NULL;
...
static void
init_screen (MsdWacomManager *manager)
{
        GdkScreen *screen;
...
static void
register_manager (MsdWacomManager *manager)
{
...
gboolean
msd_wacom_manager_start (MsdWacomManager *manager,
                         GError         **error)
{
        mate_settings_profile_start (NULL);
        ...
        manager->priv->start_idle_id = g_idle_add ((GSourceFunc) msd_wacom_manager_idle_cb, manager);
        g_source_set_name_by_id (manager->priv->start_idle_id, "[mate-settings-daemon] msd_wacom_manager_idle_cb");
...
void
msd_wacom_manager_stop (MsdWacomManager *manager)
{
        MsdWacomManagerPrivate *p = manager->priv;
        GList *l;
...
static void
msd_wacom_manager_finalize (GObject *object)
{
        MsdWacomManager *wacom_manager;

        g_return_if_fail (object != NULL);
        g_return_if_fail (MSD_IS_WACOM_MANAGER (object));

        wacom_manager = MSD_WACOM_MANAGER (object);

        g_return_if_fail (wacom_manager->priv != NULL);

        msd_wacom_manager_stop (wacom_manager);
	...
	if (wacom_manager->priv->rr_screen != NULL) {
        ...
        G_OBJECT_CLASS (msd_wacom_manager_parent_class)->finalize (object);
}

MsdWacomManager *
msd_wacom_manager_new (void)
{
        if (manager_object != NULL) {
                g_object_ref (manager_object);
        } else {
                manager_object = g_object_new (MSD_TYPE_WACOM_MANAGER, NULL);
                g_object_add_weak_pointer (manager_object,
                                           (gpointer *) &manager_object);
        }

        return MSD_WACOM_MANAGER (manager_object);
}
