...
#define MSD_WACOM_BUTTON_EDITOR_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE ((obj), MSD_WACOM_BUTTON_EDITOR_TYPE, MsdWacomButtonEditorPrivate))

G_DEFINE_TYPE (MsdWacomButtonEditor, msd_wacom_button_editor, GTK_TYPE_GRID);
...
static struct {
  MsdWacomActionType  action_type;
  const gchar        *action_name;
} action_table[] = {
  { MSD_WACOM_ACTION_TYPE_NONE,           NC_("Wacom action-type", "None")                },
  { MSD_WACOM_ACTION_TYPE_CUSTOM,         NC_("Wacom action-type", "Send Keystroke")      },
  { MSD_WACOM_ACTION_TYPE_SWITCH_MONITOR, NC_("Wacom action-type", "Switch Monitor")      },
  { MSD_WACOM_ACTION_TYPE_HELP,           NC_("Wacom action-type", "Show On-Screen Help") }
};
...
struct _MsdWacomButtonEditorPrivate
{
...
static void
assign_custom_key_to_dir_button (MsdWacomButtonEditor *self,
                                 gchar                *custom_key)
{
  MsdWacomTabletButton *button;
...
static void
change_button_action_type (MsdWacomButtonEditor *self,
                           MsdWacomActionType    type)
{
  MsdWacomActionType    current_type;
  MsdWacomTabletButton *button;
  ...
  if (button->type == WACOM_TABLET_BUTTON_TYPE_STRIP ||
      button->type == WACOM_TABLET_BUTTON_TYPE_RING)
    {
      ...
      else if (type == MSD_WACOM_ACTION_TYPE_CUSTOM)
        {
          guint           keyval;
  ...
  else if (current_type != type)
    {
      ...
      switch (type) {
      case MSD_WACOM_ACTION_TYPE_NONE:
          oled_label = "";
          break;
      case MSD_WACOM_ACTION_TYPE_HELP:
          oled_label = C_("Action type", "Show On-Screen Help");
          break;
      case MSD_WACOM_ACTION_TYPE_SWITCH_MONITOR:
  ...
  gtk_widget_set_visible (self->priv->shortcut_button,
                          type == MSD_WACOM_ACTION_TYPE_CUSTOM);
}

static void
update_action_combo (MsdWacomButtonEditor *self,
                     MsdWacomActionType    new_type)
{
  GtkTreeIter iter;
  MsdWacomActionType type;
  gboolean iter_valid;
...
static void
reset_shortcut_button_label (MsdWacomButtonEditor *self)
{
  gtk_button_set_label (GTK_BUTTON (self->priv->shortcut_button), NC_("keyboard shortcut", "None"));
}

static void
on_key_shortcut_cleared (MsdWacomKeyShortcutButton  *shortcut_button,
                         MsdWacomButtonEditor       *self)
{
  update_action_combo (self, MSD_WACOM_ACTION_TYPE_NONE);
...
static void
on_key_shortcut_edited (MsdWacomKeyShortcutButton  *shortcut_button,
                        MsdWacomButtonEditor       *self)
{
  MsdWacomTabletButton *button;
  GtkDirectionType dir;
  ...
  change_button_action_type (self, MSD_WACOM_ACTION_TYPE_CUSTOM);
...
static void
on_finish_button_clicked (GtkWidget *button, MsdWacomButtonEditor *self)
{
  g_signal_emit (self, signals[DONE_EDITING], 0);
}

static gboolean
action_type_is_allowed (MsdWacomTabletButton *button, MsdWacomActionType action_type)
{
  if (button->type != WACOM_TABLET_BUTTON_TYPE_STRIP && button->type != WACOM_TABLET_BUTTON_TYPE_RING)
    return TRUE;

  if (action_type == MSD_WACOM_ACTION_TYPE_NONE || action_type == MSD_WACOM_ACTION_TYPE_CUSTOM)
    return TRUE;
...
static void
reset_action_combo_model (MsdWacomButtonEditor *self)
{
  GtkListStore *model;
  GtkTreeIter iter;
  MsdWacomTabletButton *button;
...
static void
update_button (MsdWacomButtonEditor *self)
{
  MsdWacomTabletButton *button;
  GtkDirectionType dir;
  MsdWacomActionType current_type;
  ...
  if (button->type == WACOM_TABLET_BUTTON_TYPE_STRIP ||
      ...
      if (strv != NULL)
        {
          ...
          if (g_strcmp0 (shortcut, "") == 0)
            current_type = MSD_WACOM_ACTION_TYPE_NONE;
          else
            current_type = MSD_WACOM_ACTION_TYPE_CUSTOM;
  ...
  else
    {
      ...
      if (current_type == MSD_WACOM_ACTION_TYPE_CUSTOM)
        shortcut = g_settings_get_string (button->settings, CUSTOM_ACTION_KEY);
    }

  if (shortcut != NULL && current_type == MSD_WACOM_ACTION_TYPE_CUSTOM)
    {
...
static void
msd_wacom_button_editor_init (MsdWacomButtonEditor *self)
{
  gint i;
  ...
  priv = MSD_WACOM_BUTTON_EDITOR_GET_PRIVATE (self);
  self->priv = priv;
  ...
  shortcut_button = msd_wacom_key_shortcut_button_new ();
  ...
  g_object_set (shortcut_button,
                "mode", MSD_WACOM_KEY_SHORTCUT_BUTTON_MODE_ALL,
                "cancel-key", 0,
                NULL);
...
static gboolean
msd_wacom_button_editor_key_press (GtkWidget   *widget,
                                   GdkEventKey *event)
{
  GTK_WIDGET_CLASS (msd_wacom_button_editor_parent_class)->key_press_event (widget, event);

  return FALSE;
}

static void
msd_wacom_button_editor_class_init (MsdWacomButtonEditorClass *klass)
{
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

  signals[BUTTON_EDITED] = g_signal_new ("button-edited",
                                         MSD_WACOM_BUTTON_EDITOR_TYPE,
                                         G_SIGNAL_RUN_LAST,
                                         G_STRUCT_OFFSET (MsdWacomButtonEditorClass, button_edited),
                                         NULL, NULL,
  ...
  signals[DONE_EDITING] = g_signal_new ("done-editing",
                                         MSD_WACOM_BUTTON_EDITOR_TYPE,
                                         G_SIGNAL_RUN_LAST,
                                         G_STRUCT_OFFSET (MsdWacomButtonEditorClass, done_editing),
                                         NULL, NULL,
                                         g_cclosure_marshal_VOID__VOID,
                                         G_TYPE_NONE, 0);

  widget_class->key_press_event = msd_wacom_button_editor_key_press;

  g_type_class_add_private (klass, sizeof (MsdWacomButtonEditorPrivate));
}

void
msd_wacom_button_editor_set_button (MsdWacomButtonEditor *self,
                                    MsdWacomTabletButton *button,
                                    GtkDirectionType      direction)
{
  gboolean reset = TRUE;

  g_return_if_fail (MSD_WACOM_IS_BUTTON_EDITOR (self));
...
msd_wacom_button_editor_get_button (MsdWacomButtonEditor *self,
                                    GtkDirectionType     *direction)
{
  g_return_val_if_fail (MSD_WACOM_IS_BUTTON_EDITOR (self), NULL);
...
GtkWidget *
msd_wacom_button_editor_new (void)
{
  return g_object_new (MSD_WACOM_BUTTON_EDITOR_TYPE, NULL);
}
